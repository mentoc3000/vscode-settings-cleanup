...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('settings-organizer.organize', () => {
		...
		const organizedSettings = organizeSettings(file || '', spacing || defaultSpacing);
		// Replace contents of file
		vscode.window.activeTextEditor?.edit(editBuilder => {
			editBuilder.replace(
				new vscode.Range(new vscode.Position(0, 0),
				new vscode.Position(vscode.window.activeTextEditor?.document.lineCount || 0, 0)),
				organizedSettings);
		});
	});
